...
export const
  appLoaded = (state = false, action)=> {
    if (action.type == ActionType.APP_LOADED){
      return true
    } else {
      return state
    }
  ...
  disconnected = (state = false, action)=> {
    ...
    if (action.type === ActionType.DISCONNECTED){
      return true
    }
    return state
  },

  isDemo = (state = false, { type }) => type == ActionType.START_DEMO ? true : state,

  demoDownloadUrl = (state = null, {type, payload})=> {
    return type == ActionType.SET_DEMO_DOWNLOAD_URL ? payload : state
  },
  ...
  isInvitee = (state = false, action)=>{
    ...
    switch (action.type){
      case ActionType.ACCEPT_INVITE_SUCCESS:
        return true
  ...
  generatedInviteLinks = (state={}, action)=>{
    if (action.type == ActionType.CLOSE_GENERATED_INVITE_LINK){
  ...
  versionFilters = (state = {}, action)=> {
    ...
    if (action.type == ActionType.UPDATE_VERSION_FILTERS){
